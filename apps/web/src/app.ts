(() => {
  ...
  function renderMethodology(): void {
    const activeLevel = state.mode === 'synthetic_fixture'
      ? 'You are viewing <strong>Level 1 material</strong>: a synthetic fixture with authored findings.'
      : 'You are viewing <strong>Level 2 output</strong>: heuristic, unbenchmarked local preview of your pasted text.';
    methodologyView.innerHTML = `<section class="section-shell methodology"><div class="section-heading"><div><span class="eyebrow">Methodology &amp; transparency</span><h1>An instrument, not a verdict machine.</h1></div></div><div class="method-grid"><article><h2>Interpretive pressure</h2><p>Pressure measures how strongly language steers or narrows interpretation. It is not factuality, political side, moral worth, or harm.</p></article><article><h2>Confidence is separate</h2><p>A strong-pressure finding can be low confidence; a light-pressure finding can be high confidence. Candidates remain visible without inflating confirmed metrics.</p></article><article><h2>Comparison is evidence-bound</h2><p>Material omission requires aligned event context, chronology, supporting coverage or evidence, materiality, and proof the target does not already state the equivalent fact.</p></article><article><h2>No hidden chain-of-thought</h2><p>The system exposes exact spans, criteria, exclusions, alternatives, provenance, and versioned records rather than private reasoning traces.</p></article></div><div class="method-grid"><article><h2>Which detector produced this?</h2><p>${activeLevel}</p><p><strong>Level 1 — Experience Prototype:</strong> deterministic synthetic demonstration. Not a calibrated instrument.</p><p><strong>Level 2 — Local Preview:</strong> heuristic rules on real pasted text. Unbenchmarked, non-authoritative, candidates only. This is what runs in this page.</p><p><strong>Level 3 — Instrument Alpha:</strong> the taxonomy-governed detector in <code>services/rhetoric</code>. It is <em>not</em> wired into this browser build, because that requires a service boundary that has been deliberately deferred.</p></article><article><h2>Calibration status</h2><p><strong>Uncalibrated.</strong> No human-reviewed benchmark has been run against any detector level in this repository. No precision, recall or accuracy figure exists, and none is displayed anywhere in this interface.</p><p>Everything shown here is candidate rhetorical analysis for human review, not a validated measurement.</p></article><article><h2>What this build cannot do</h2><p>No network requests. No URL ingestion. No live coverage retrieval. No fact checking. No evidence authentication. Cross-document comparison, Event Record and Material Omission are available only for the synthetic fixture, which carries reserved <code>example.invalid</code> provenance.</p></article><article><h2>Versions</h2><p>Taxonomy <code>${esc(DATA.taxonomy.version)}</code> · ${DATA.taxonomy.mechanisms.length} mechanisms defined, ${DATA.vocabulary.intrinsicAlphaSlice.length} implemented in the local preview slice.</p><p>Analysis run <code>${esc(state.analysisRun?.runId || 'none')}</code> · stage <code>${esc(state.analysisRun?.stage || 'none')}</code> · detector <code>${esc(state.analysisRun?.detectorVersion || 'none')}</code>.</p></article></div></section>`;
  }
